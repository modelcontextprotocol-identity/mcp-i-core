/**
 * The merchant's audit trail — the SHIPPED `@kya-os/mcp/audit` protocol, wired
 * into the same middleware that gates `place_order`, so every event the room
 * watches (the order, the refusals, the theft, the kill, the retry) is recorded
 * by the SDK's MCP adapter, not by demo code:
 *
 *   - each entry is signed by the merchant's key (a recorder receipt) and
 *     hash-chained to its predecessor (previousEntryDigest)
 *   - a checkpoint is an RFC 9162 Merkle root over the entry digests, signed
 *   - any entry proves its inclusion against that root with an audit path
 *   - the Responsible Party WITNESSES each checkpoint (a signed observation
 *     receipt), so the merchant cannot later publish a different history
 *   - the whole thing exports as the SDK's replay bundle, which the SDK's own
 *     offline verifier (`npx kya-audit verify`) and a stdlib-Python script
 *     (scripts/verify-ledger.py) re-check without trusting this process
 *
 * This module adds the demo's reads on top: render the ledger, the tree, the
 * checkpoint, the witness and the inclusion proofs; simulate an INSIDER WITH
 * THE MERCHANT KEY editing one entry, then show what still breaks.
 *
 * Honest labelling: the journal here is in-memory, so the SDK refuses to let
 * this deployment advertise more than AAP-1 ("Recorded"); the same code on a
 * durable journal is AAP-3, and with the witness it is AAP-4. The assertion
 * that enforces this (`assertAuditCapabilities`) runs at startup.
 */
...
import { VAR_DIR, type KeyedIdentity } from '../lib/wiring.js';
import { jwkFor, signingKeyFor } from '../lib/keys.js';
...
export interface MerchantAudit {
  ...
  /** Record a merchant-side decision (the handler's own gates) on the same ledger. */
...
export interface MerchantAuditOptions {
  ledgerId?: string;
  epochId?: string;
...
export async function createMerchantAudit(identity: KeyedIdentity, options: MerchantAuditOptions = {}): Promise<MerchantAudit> {
  ...
  const ledger = {
    ledgerId: options.ledgerId ?? `kya:merchant:${shortId(identity.did)}:orders`,
    ledgerEpochId: options.epochId ?? `epoch-${new Date(clock.now()).toISOString().slice(0, 10)}`,
  };
  const tenantRef: PartyRef = { kind: 'keyed_commitment', value: `sha256:${createHash('sha256').update(identity.did).digest('hex')}`, keyId: 'merchant-tenant-v1' };
  ...
  const recorder = createLocalAuditRecorder({
    ...
    sourceId: 'merchant-edge',
    journal,
    signer,
    hasher,
    clock,
  }, () => ({ producerAuthority: identity.did, tenantAuthority: 'merchant', tenantRef }));
  ...
  const trail = createAuditTrail({
    ...
    tenantRef,
    producer: { kind: 'pairwise_did', did: identity.did },
    sourceId: 'merchant-edge',
    binding: AUDIT_BINDING,
  ...
  async function tamper(): Promise<TamperReport> {
    ...
    return {
      ...
      attacker: 'insider with the merchant signing key and write access to the journal',
      anchoredRoot: root,
      tamperedRoot,
